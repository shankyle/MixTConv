...
def parse_operation_from_pth_name(weight_path):
    strings = weight_path.split('/')[1]
    if'_nl' in strings:
        is_nl = True
    else:
        is_nl = False
    if'_inplace' in strings:
        is_inplace = True
    else:
        is_inplace = False
    ...
    if 'RGB' in strings:
        ...
        modality = 'Flow'
    # embed()
    if 'ms' not in strings:
        operation = strings[strings.index('ops') + 1]
    else:
        operation = 'ms_' + strings[strings.index('ops') + 2]
    n_div = int(strings[strings.index(operation.split('_')[-1]) + 1])
    for i, s in enumerate(strings):
        if 'corrg' in s:
            corrg = int(s.split('corrg')[-1])
        else:
            corrg = 1
    return operation, n_div, is_dwise, corrg, is_nl, modality, is_inplace
...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    operation, n_div, is_dwise, corrg, is_nl, modality, is_inplace = parse_operation_from_pth_name(this_weights)
    this_arch = this_weights.split(modality+'_')[1].split('_')[0]
    modality_list.append(modality)
    num_class, args.train_list, val_list, root_path, prefix = dataset_config.return_dataset(args.dataset,
                                                                                            modality)
    print('=> operation: {}, n_div: {}, is_dwise: {}, corrg: {}, is_inplace: {}'.format(operation, n_div, is_dwise, corrg, is_inplace))
    net = TSN(num_class, this_test_segments if operation is not None else 1, modality,
              base_model=this_arch,
              consensus_type=args.crop_fusion_type,
              pretrain=args.pretrain,
              operations=operation, n_div=n_div, dwise=is_dwise, corr_g=corrg, inplace=is_inplace,
              non_local=is_nl,
              )
    # embed()
...
def eval_video(video_data, net, this_test_segments, modality):
    ...
    with torch.no_grad():
        ...
        if net.module.operations is not None:
