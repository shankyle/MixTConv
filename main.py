...
from ops.utils import AverageMeter, accuracy, EarlyAjust, EarlyAjustAc
...
def main():
    ...
    if args.operations:
        full_arch_name += '_ops_{}_{}'.format(args.operations, args.n_div)
    ...
    if args.lr_type == 'earlyadj':
        lr_adj = EarlyAjust(patience=5, delta=0)
    elif args.lr_type == 'earlyadjac':
        lr_adj = EarlyAjustAc(patience=5, delta=0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        adjust_learning_rate(optimizer, epoch, args.lr_type, args.lr_steps,
                             lr_adj.earlyadj if 'earlyadj' in args.lr_type else 0)
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            prec1, losses = validate(val_loader, model, criterion, epoch, log_training, tf_writer)

            # remember best prec@1 and save checkpoint
            if args.lr_type == 'earlyadj':
                lr_adj(losses)
            if args.lr_type == 'earlyadjac':
                lr_adj(prec1)
...
def validate(val_loader, model, criterion, epoch, log=None, tf_writer=None):
    ...
    return top1.avg, round(losses.avg, 5)
...
def adjust_learning_rate(optimizer, epoch, lr_type, lr_steps, lr_adj=0):
...
if __name__ == '__main__':
    ...
    # while memory_gpu[4] < 32000 or memory_gpu[5] < 32000 or memory_gpu[6] < 32000 or memory_gpu[7] < 32000:
