...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            if self.operations == 'ms_cenearres':
                import arch as ac
                self.base_model = getattr(ac, base_model)(True)
            else:
                self.base_model = getattr(torchvision.models, base_model)(True)
            if self.operations is not None:
                print('Adding group 1d convolution')
                from ops.operations import make_operations
                make_operations(self.base_model, self.num_segments, n_div=self.n_div, operations=self.operations,
                                dwise=self.dwise, corr_group=self.corr_g, inplace=self.inplace)
    ...
    def _construct_flow_model(self, base_model):
        ...
        setattr(container, layer_name, new_conv)  # 添加新类属性
